...
/****************************
purpose: fast to draw a picture
****************************/

void LCD_ShowPicture_DMA(const unsigned char pic[]) {
	
    while (HAL_SPI_GetState(&hspi1) == HAL_SPI_STATE_BUSY_TX) {};
    LCD_Address_Set(0, 0, 240, 240); 
    LCD_CS_Clr();  
    LCD_DC_Set();  
    HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)pic , 57600); 
    while (HAL_SPI_GetState(&hspi1) == HAL_SPI_STATE_BUSY_TX) {};
		LCD_CS_Clr();
    HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)(pic + 57600), 57600); 
}
